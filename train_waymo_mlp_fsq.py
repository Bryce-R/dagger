"""Train an MLP-FSQ trajectory autoencoder on Waymo Motion Scenario TFRecords.
...
them into each agent's local frame, and trains a fixed-horizon MLP autoencoder
with finite scalar quantization on normalized motion deltas.

Example:
  # From the dagger repo root:
  python train_waymo_mlp_fsq.py \
    --tfrecord /path/to/uncompressed_scenario_training_training.tfrecord-00000-of-01000 \
    --device mps \
    --epochs 20

  # Long Waymo vehicle MLP-FSQ run from the dagger repo root, using the TFRecord
  # stored in the local waymo-open-dataset checkout. This is the default path
  # for future Waymo trajectory compression runs; the older VQ-VAE/conv options
  # intentionally are not exposed here.
  env MPLCONFIGDIR=/private/tmp/mplconfig XDG_CACHE_HOME=/private/tmp \
    python train_waymo_mlp_fsq.py \
      ...
      --plot-every 1000 \
      --log-every 1000 \
      --plot-dir waymo_mlp_fsq_dynamicstates_h512_cd64_tok1_posw0_e12000_b512
  ...
  WAYMO_OPEN_DATASET_ROOT=/Users/pengkai/Code/waymo-open-dataset \
    python train_waymo_mlp_fsq.py --tfrecord /path/to/scenario.tfrecord
"""
...
class TrajectoryMLPFSQ(nn.Module):
  """MLP autoencoder with finite scalar quantized latents for [B, T, D]."""
...
def save_reconstruction_plot(
  ...
  fig.suptitle(
      f"Waymo trajectory MLP-FSQ {split_name} reconstruction, epoch {epoch}, "
...
def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
      description="Train an MLP-FSQ autoencoder on Waymo Motion future trajectories."
  )
  ...
  parser.add_argument(
      ...
      type=int,
      default=0,
      help="Train with continuous tanh latents for this many epochs before FSQ.",
  )
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      type=int,
      default=0,
      help="Print encoder/pre-tanh/bin usage stats every N epochs.",
  )
  parser.add_argument(
  ...
  parser.add_argument("--plot-dir", default="waymo_mlp_fsq_plots")
  parser.add_argument(
      "--plot-pdf",
      default=None,
      help=(
